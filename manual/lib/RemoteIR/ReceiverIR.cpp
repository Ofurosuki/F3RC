...
void ReceiverIR::init_state(void) {
  ...
  timer.stop();
  timer.reset();
  for (unsigned int i = 0; i < sizeof(data.buffer); i++) {
    data.buffer[i] = 0;
  }
}
...
void ReceiverIR::isr_wdt(void) {
  ...
  static int cnt = 0;
  if ((Idle != work.state) ||
      ((0 <= work.c1) || (0 <= work.c2) || (0 <= work.c3) || (0 <= work.d1) ||
       (0 <= work.d2))) {
    cnt++;
    if (cnt > 50) {
#if 0
  ...
  switch (work.state) {
    case Idle:
      if (work.c1 < 0) {
        ...
        if (InRange(a, RemoteIR::TUS_NEC * 16) &&
            InRange(b, RemoteIR::TUS_NEC * 8)) {
          /*
           * NEC.
           */
          ...
          data.bitcount = 0;
        } else if (InRange(a, RemoteIR::TUS_NEC * 16) &&
                   InRange(b, RemoteIR::TUS_NEC * 4)) {
          /*
           * NEC Repeat.
           */
          ...
          work.d1 = -1;
          work.d2 = -1;
        } else if (InRange(a, RemoteIR::TUS_AEHA * 8) &&
                   InRange(b, RemoteIR::TUS_AEHA * 4)) {
          /*
           * AEHA.
           */
    ...
    case Receiving:
      if (RemoteIR::NEC == data.format) {
         ...
         */
        timeout.detach();
        timeout.attach_us(callback(this, &ReceiverIR::isr_timeout),
                          RemoteIR::TUS_NEC * 5);
#endif
